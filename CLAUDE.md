...
- Never run `gh auth login`, `gh auth refresh` or `gh auth switch`, and never ask the user to log in. For `gh` commands use the permanent token in `~/.config/coney/github-token`: `GH_TOKEN=$(cat ~/.config/coney/github-token) gh ...`. If GitHub rejects something, report the error and stop.
